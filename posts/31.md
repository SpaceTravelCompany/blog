...
- `layout`: descriptor set layout + push constant range (파이프라인 레이아웃은 [28번 글](https://xfitgd.github.io/blog/posts/28) 참고)
...
Mesh 셰이더는 `gl_MeshVerticesEXT[]` 배열을 통해 정점을 생성하고, `gl_PrimitiveMeshEXT[]`로 프리미티브(삼각형/라인/점)를 정의한다. 전통적인 버텍스 버퍼가 필요 없어서 **GPU 드리븐 렌더링**이나 **동적 LOD**에 유리하다.

---

## 참고 출처

- **Vulkan Specification – 10.4. Graphics Pipelines**  
  https://docs.vulkan.org/spec/latest/chapters/graphicspipelines.html  
  (VkGraphicsPipelineCreateInfo 전체, 4가지 state 그룹, VU 목록)

- **Vulkan Specification – VkPipelineVertexInputStateCreateInfo**  
...
- **Vulkan Specification – VkPipelineRasterizationStateCreateInfo**  
  https://docs.vulkan.org/refpages/latest/refpages/source/VkPipelineRasterizationStateCreateInfo.html  
  (Cull mode, polygon mode, depth bias)

- **Vulkan Specification – VkPipelineColorBlendStateCreateInfo**  
  https://docs.vulkan.org/refpages/latest/refpages/source/VkPipelineColorBlendStateCreateInfo.html  
  (Blend equation, logic op, attachment별 설정)

- **Vulkan Specification – 10.4. Graphics Pipeline Shader Groups**  
  https://docs.vulkan.org/spec/latest/chapters/graphicspipelines.html#VkGraphicsPipelineShaderGroupsCreateInfoNV  
  (NV_device_generated_commands shader group)
