...
```cmdstack
lua_getglobal(L, "play_card") → [play_card]
lua_pushinteger(L, 1) → [play_card, 1]
lua_pushinteger(L, 2) → [play_card, 1, 2]
---
...
```cmdstack
lua_getglobal(L, "get_score") → [get_score]
---
...
```cmdstack
lua_getglobal(L, "get_pos") → [get_pos]
