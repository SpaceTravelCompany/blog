...
-> gpu 를 블로킹하는것이기 때문에 cpu 쪽 코드 실행에서 대기 하는게 아니고 따라서 wait 같은 함수가 없는것이다.. 지만 밑의 Timeline Semaphore는 예외
...
Pipeline Barrier는 가장 자주 사용되는 동기화 수단이다. 커맨드 버퍼 기록 시점에 **어떤 스테이지들의 작업이 끝나야 다음 스테이지들이 시작될 수 있는지**를 지정한다.
...
## 7. 타임라인 예제 (Triple Buffering + Swapchain)
