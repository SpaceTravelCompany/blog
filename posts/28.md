...
기본적으로 descriptor set은 **bind 이후에 업데이트해도 반영되지 않는다**. `VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT`를 사용하면 바인딩 후에도 descriptor를 업데이트할 수 있다.
...
- **Vulkan Specification – 15.2. Descriptor Sets**  
  https://docs.vulkan.org/spec/latest/chapters/descriptorsets.html  
  (DescriptorSetLayout, PipelineLayout, Pool, Set 생성/할당 전반)

- **Vulkan Specification – VkDescriptorSetLayoutCreateInfo**  
  https://docs.vulkan.org/refpages/latest/refpages/source/VkDescriptorSetLayoutCreateInfo.html  
  (Layout binding 정의 방식)

- **Vulkan Specification – VkPipelineLayoutCreateInfo**  
  https://docs.vulkan.org/refpages/latest/refpages/source/VkPipelineLayoutCreateInfo.html  
  (Push constant 포함)

- **Vulkan Spec – VkPhysicalDeviceLimits** (maxBoundDescriptorSets 등)  
